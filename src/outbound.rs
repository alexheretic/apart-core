...
impl ToYaml for CloneStatus {
    fn to_yaml(&self) -> String {
        match *self {
            ...
            CloneStatus::Finished { ref finish, ref common, image_size } => format!(
                "type: clone\n\
                 {common_yaml}\n\
                 complete: 1.0\n\
                 syncing: false\n\
            ...
            CloneStatus::Failed { ref finish, ref common, ref reason } => format!(
                "type: clone-failed\n\
                 {common_yaml}\n\
                 finish: {finish:?}\n\
                 error: {error}",
...
impl<'a> ToYaml for RestoreStatus<'a> {
    fn to_yaml(&self) -> String {
        match *self {
            ...
            RestoreStatus::Failed { ref common, ref reason, finish } => format!(
                "type: restore-failed\n\
                 {common_yaml}\n\
                 finish: {finish:?}\n\
                 error: {error}",
...
mod tests {
    ...
    fn status_started_yaml() {
      ...
      {"name": "sda", "size": "750156374016", "fstype": null, "label": null, "mountpoint": null,
        ...
        let yaml =
            YamlLoader::load_from_str(&status_yaml("started", lsblk_json)).unwrap().remove(0);
