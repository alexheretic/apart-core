...
fn do_clone_job() {
    ...
    let expected_filename =
        format!("do_clone_job-{}.apt.dd.gz", Local::now().format("%Y-%m-%dT%H%M"));
    ...
    let ref msg = core.expect_message_with(
        |msg| msg["type"].as_str() == Some("clone") && msg["rate"].as_str().is_some(),
    );
    ...
    assert_eq!(
        core.get_tmp_file_contents_utf8(".latest.s.mockpcl.dd.txt").expect("!last source"),
        "/dev/sda5"
    );
    assert!(
    ...
    let output = core.get_tmp_file_contents_bytes(&expected_filename)
        .expect("!read $tmp_dir/do_clone_job.apt.gz");
    assert_eq!(decompress_gz(&output).expect("!decompress"), "mock-partition-/dev/sda5-data");
...
fn clone_using_partclone_fstype_variant_f2fs() {
    ...
    core.send(&clone_msg);
    let expected_filename =
        format!("f2fs_job-{}.apt.f2fs.gz", Local::now().format("%Y-%m-%dT%H%M"));
    ...
    assert_eq!(
        core.get_tmp_file_contents_utf8(".latest.s.mockpcl.f2fs.txt").expect("!last source"),
        "/dev/sdb3"
    );
    assert!(
...
fn clone_using_partclone_fstype_variant_ext2() {
    ...
    core.send(&clone_msg);
    let expected_filename =
        format!("ext2_job-{}.apt.ext2.gz", Local::now().format("%Y-%m-%dT%H%M"));
    ...
    assert_eq!(
        core.get_tmp_file_contents_utf8(".latest.s.mockpcl.ext2.txt").expect("!last source"),
        "/dev/sdb1"
    );
    assert!(
...
fn clone_and_compress_with_zstd() {
    ...
    core.send(&clone_msg);
    let expected_filename =
        format!("zst_job-{}.apt.ext2.zst", Local::now().format("%Y-%m-%dT%H%M"));
...
fn clone_and_compress_with_lz4() {
    ...
    core.send(&clone_msg);
    let expected_filename =
        format!("lz4_job-{}.apt.ext2.lz4", Local::now().format("%Y-%m-%dT%H%M"));
...
fn clone_uncompressed_uuid_info() {
    ...
    core.send(&clone_msg);
    let expected_filename =
        format!("no_z_job-{}.apt.ext2.uncompressed", Local::now().format("%Y-%m-%dT%H%M"));
...
fn handle_partclone_rate_output() {
    ...
    core.set_mock_partclone(
        "ext2",
        MockPartcloneState::new().complete(0.6).rate("Rate: 2.34GB/min"),
    ).expect("!set_mock_partclone");
    let ref msg = core.expect_message_with(|msg| msg["complete"].as_f64() != Some(0.5));
...
fn delete_image_not_found() {
    ...
    core.send(&clone_msg);
    let ref msg =
        core.expect_message_with(|msg| msg["type"].as_str() == Some("delete-clone-failed"));
    assert_eq!(msg["file"].as_str().unwrap(), image);
